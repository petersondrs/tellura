...
function render_megamenu_items($menies, $parent, $parent_meny){
    $html = '';
    foreach($menies as $meny){
        if( $meny->menu_item_parent == $parent ){
    ...
    $ul_style = "";
    if( $parent_meny->activemega=='1' && trim($parent_meny->image)!='' ){
        $ul_style .= "background-image:url($parent_meny->image);";
        if( !empty($parent_meny->bgprop) ){
            $ar = explode('|', $parent_meny->bgprop);
            if( count($ar)>1 ){
                $ul_style .= "background-position:$ar[1];";
                if( $ar[0]=='cover' || $ar[0]=='contain' ){
                    $ul_style .= "background-size:$ar[0]; background-repeat:no-repeat;";
                }
                else{
                    $ul_style .= "background-repeat:$ar[0];";
                }
            }
        }
    }
    $mega_ul_class = '';
    $mega_ul_class .= !empty($parent_meny->image) ? 'mega-menu-bg-image' : '';

    return $html!='' ? ('<ul class="dropdown-menu '.$mega_ul_class.'" style="'.$ul_style.'"><li>'.$html.'<div class="clearfix"></div></li></ul>') : '';
}
...
function ttmegamenu_render_scripts($hook) {
	...
	wp_enqueue_script('tt_admin_option_script', file_require(get_template_directory_uri().'/framework/mega-menu/mega-menu.js', true), false, false, true);
}
...
add_filter( 'wp_setup_nav_menu_item','bgprop_nav_item' );
function bgprop_nav_item($menu_item) {
    $menu_item->bgprop = get_post_meta( $menu_item->ID, '_menu_item_bgprop', true );
    return $menu_item;
}
add_action('wp_update_nav_menu_item', 'bgprop_nav_update',10, 3);
function bgprop_nav_update($menu_id, $menu_item_db_id, $args ) {
    ob_start();
    if ( isset($_REQUEST['menu-item-bgprop']) && is_array($_REQUEST['menu-item-bgprop']) ) {
        $items = $_REQUEST['menu-item-bgprop'];
        $custom_value = $items[$menu_item_db_id];
        update_post_meta( $menu_item_db_id, '_menu_item_bgprop', $custom_value );
    }
    ob_clean();
}
    ...
    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
            ...
            <p class="field-image description description-wide">
                ...
                <span class="field-mega-menu-options" style="margin-top:10px; margin-bottom:10px; display:none;">
                    <select class="field-mm-image-prop-repeat">
                    ...
                    <select class="field-mm-image-prop-position">
                    ...
                    <input type="hidden" id="edit-menu-item-bgprop-<?php echo $item_id; ?>" class="widefat code edit-menu-item-bgprop" name="menu-item-bgprop[<?php echo $item_id; ?>]" value="<?php echo esc_attr( $item->bgprop ); ?>" />
                    <span></span>
                </span>
            </p>
