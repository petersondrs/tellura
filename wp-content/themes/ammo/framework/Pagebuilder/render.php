            ...
            if (blox_post_type() == $value) {
                ...
                foreach ($blox_items as $item){
                    if( isset($item['js']) && $item['js']!='' ){
                        ...
                        wp_enqueue_script('blox_render_scripts'.$file_index, file_require(BLOX_PATH.'items/'.$item['js'], true), false, false, true);
                        $file_index++;
                    }
                }
                //$minify = new TTMinify();
...
/* Include Fonts */
require_once file_require(BLOX_DIR . 'font-reader.php');
